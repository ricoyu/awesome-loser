 ...
 */
@Slf4j
public final class RestMultiGetBuilder<T> {
	...
	public RestMultiGetBuilder(RestHighLevelClient client) {
		this.client = client;
	}
	
	public RestMultiGetBuilder add(String index, String id) {
		items.add(new Item(index, id));
		return this;
	}
	
	public RestMultiGetBuilder add(String index, List<String> ids) {
		ids.stream().map(id -> new Item(index, id))
				.forEach(item -> items.add(item));
		return this;
	}
	
	public RestMultiGetBuilder add(Map<String, String> params) {
	...
	public RestMultiGetBuilder resultType(Class<T> clazz) {
		this.clazz = clazz;
		return this;
	}
