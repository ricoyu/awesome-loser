...
public abstract class AbstractQueuedSynchronizer
	 ...
	 * <p>
	 * 不管是条件队列, 还是CLH队列, 都是基于这个Node对象去构造的一个数据结构
	 */
	static final class Node {
		...
		/**
		 * 节点在条件队列中，节点的线程等待在Condition上，当其他线程对Condition调用了signal()方法后，
		 * 该节点会从条件队列中转移到同步队列中，加入到同步状态的获取中
		 * <p>
		 * 如果节点在条件队列中, Node必须是独占模式(不能是共享模式)
		 */
	 ...
	 * <p>
	 * 记录锁被加了多少次(锁的上锁次数)
	 * lock.lock() 成功 state == 1; 再次lock.lock()成功 state == 2
	 */
	...
	private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
		...
		if (ws > 0) {
			/*
			 * waitStatus > 0 只有一种状态, 即 1. 表示线程等待超时或者被中断了
	...
	public class ConditionObject implements Condition, java.io.Serializable {
		...
		private void unlinkCancelledWaiters() {
			Node t = firstWaiter; //t指向条件队列第一个节点
			Node trail = null;
			while (t != null) {
				Node next = t.nextWaiter;
                if (t.waitStatus != Node.CONDITION) {//如果头结点不是CONDITION状态
                    t.nextWaiter = null;
                    if (trail == null) {
