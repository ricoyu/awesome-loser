...
import com.loserico.networking.wrapper.WrapperFactory;
import com.loserico.networking.wrapper.WrapperFactory.WrapperFactoryEnum;
...
public abstract class AbstractRequestBuilder {
	...
	{
		wrapperFactory = WrapperFactoryEnum.INSTANCE.getInstance();
	}
	...
	protected CloseableHttpClient buildHttpClient() {
		...
		if (this.retryCount != null) {
			builder.setRetryHandler(
					new DefaultHttpRequestRetryHandler(Math.min(this.retryCount, 3),
							false));
		}
	...
	/**
	 * Sets retry count if request failure.
	 *
	 * @param count Notice: the maximum number of retries is 3，
	 *              so if you sets it greater than 3，it will force a change to 3.
	 * @return
	 */
	public AbstractRequestBuilder retryCount(int count) {
	 ...
	 * @return
	 */
	public AbstractRequestBuilder timeout(RequestTimeout timeout) {
		this.wrapperFactory.initTimeout(timeout);
		return this;
	}
	...
	protected HttpUriRequest buildHttpRequest(URIBuilder builder) {
		...
		request = wrapperFactory.startWrap(method);
