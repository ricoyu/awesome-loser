...
public final class NetUtils {
	...
	public static boolean isValidIpV4(String ip) {
		...
		int subnetMask = Integer.parseInt(ipSubnetMaskPair[1]);
		...
		/*
		 * 再结合子网掩码部分判断IP是否合法
		 * IPv4总共32位, 减去subnetMask表示后多少位必须为0
		 */
		int zeroBits = 32 - subnetMask;
		
		/*
		 * 把IP地址各部分转成二进制, 每部分占8位, 不足8位的前面补0
		 */
		String[] subIps = ip.split("\\.");
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < subIps.length; i++) {
			sb.append(toBinaryStr(Integer.parseInt(subIps[i])));
		}
		
		/*
		 * 拿到IP地址的二进制表示后, 从后往前数zeroBits位, 检查每一位上是不是都是'0'
		 * 此时IP地址的二进制表示是一个32位的1,0串, 所以下面从后往前循环的话i从31开始
		 */
		for (int i = 31; i > 31 - zeroBits; i--) {
			char c = sb.charAt(i);
			if (c != '0') {
				return false;
			}
	...
	public static boolean isValidIpV6(String ip) {
		...
		/*
		 * 再结合子网掩码部分判断IP是否合法
		 * IPv6总共128位, 减去subnetMask表示后多少位必须为0
		 */
		int zeroBits = 128 - subnetMask;
		
		/*
		 * IPv6 地址由 8 组 16 进制的数字来表示, 把这8组16进制数字转成二进制, 每部分占16位, 不足16位的前面补0
		 */
		String[] subIps = ip.split(":");
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < subIps.length; i++) {
			sb.append(toBinaryStr(subIps[i]));
		}
		
		/*
		 * 如果IPv6地址为 2001:0db8:3c4d:0015::/64 这种形式
		 * 那么不满8组, 所以后面要补全8组, 每组16个0
		 */
		int remainGroups = 8 - subIps.length;
		if (remainGroups > 0) {
		...
		/*
		 * 拿到IP地址的二进制表示后, 从后往前数zeroBits位, 检查每一位上是不是都是'0'
		 * 此时IP地址的二进制表示是一个128位的1,0串, 所以下面从后往前循环的话i从127开始
		 */
		for (int i = 127; i > 127 - zeroBits; i--) {
			char c = sb.charAt(i);
			if (c != '0') {
				return false;
			}
